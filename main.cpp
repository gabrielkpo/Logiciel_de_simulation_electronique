...
std::unique_ptr<Source> initialiserSource(bool useDefaults, double &A, double &f, double &off);
...
int main() {
    ...
    char useDefChar; cin >> useDefChar;
    ...
    if (!useDefaults) {
        ...
        cout << "Utilisation des paramètres par défaut (Circuit A, méthode Euler, R=1000, C=1e-6, etc.)" << endl;
    }
    ...
    // initialisation à 1 par défaut et lecture réponse utilisateur
    int choixMeth = 1;
    if (!useDefaults) cin >> choixMeth;
    ...
    if (!useDefaults) {
        ...
        if (choixCircuit == 'B') {
            double R1 = 1000.0;
            cout << "R1 (Ohms) ? [1000] "; if(!(cin >> R1)) { cin.clear(); cin.ignore(numeric_limits<streamsize>::max(), '\n'); R1=1000.0; }
            cout << "R2 (Ohms) ? [1000] "; if(!(cin >> R2)) { cin.clear(); cin.ignore(numeric_limits<streamsize>::max(), '\n'); R2=1000.0; }
            R = R1;
            cout << "C (Farads) ? [1e-6] "; if(!(cin >> C)) { cin.clear(); cin.ignore(numeric_limits<streamsize>::max(), '\n'); C=1e-6; }

        } else {
            cout << "R (Ohms) ? [1000] "; if(!(cin >> R)) { cin.clear(); cin.ignore(numeric_limits<streamsize>::max(), '\n'); R=1000.0; }
            cout << "C (Farads) ? [1e-6] "; if(!(cin >> C)) { cin.clear(); cin.ignore(numeric_limits<streamsize>::max(), '\n'); C=1e-6; }

            if(choixCircuit=='C' || choixCircuit=='D') {
                cout << "L (Henrys) ? [1e-3] "; if(!(cin >> L)) { cin.clear(); cin.ignore(numeric_limits<streamsize>::max(), '\n'); L=1e-3; }
            }
        }
    } else {
    ...
    switch(choixCircuit) {
        case 'A': circuitPtr = make_unique<CircuitA>(R, C, f); break;
        case 'B': circuitPtr = make_unique<CircuitB>(R, C, f); break; 
        case 'C': circuitPtr = make_unique<CircuitC>(R, C, L, f); break;
        case 'D': circuitPtr = make_unique<CircuitD>(R, C, L, f); break;
        default:  cout << "Choix invalide, défaut A" << endl; circuitPtr = make_unique<CircuitA>(R, C, f); break;
    }
    ...
    double Vin = source->getValue(t);
        ...
        // On applique la méthode numérique choisie suivant le type d'ordre du circuit
        if (circuitPtr->order() == 1) {

            // 1er ordre : wrapper f1

            ctx.x1 += sim.getDt() * ctx.f1(t, ctx.x1);
        } else {
    ...
    if (fabs(ctx.x1) < 1e-12) ctx.x1 = 0.0;
    if (fabs(ctx.x2) < 1e-12) ctx.x2 = 0.0;
